...
var Walker = (function (_super) {
    ...
    function Walker(sourceFile, rawOptions, program) {
        ...
        if (options) {
            ...
            _this.allowMethods = (options.allowMethods !== undefined) ? options.allowMethods : _this.allowMethods;
            _this.allowParameters = (options.allowParameters !== undefined) ? options.allowParameters : _this.allowParameters;
            _this.allowProperties = (options.allowProperties !== undefined) ? options.allowProperties : _this.allowProperties;
    ...
    Walker.prototype.isUnsafe = function (node) {
        ...
        if (util_1.isWithinCallExpressionExpression(node) || tsutils.isTaggedTemplateExpression(node.parent)) {
            if (util_1.isThis(node)) {
        ...
        if (tsutils.isPropertyAccessExpression(node.parent)) {
            if (tsutils.isClassDeclaration(declaration)) {
                var nameDeclaration = util_1.findDeclaration(node.parent.name, typeChecker);
                if (!nameDeclaration) {
                    return false;
                }
            ...
            else if (util_1.isThis(node)) {
                return !this.allowProperties;
            }
            else if (node === node.parent.name) {
                return false;
            }
            var nameType = typeChecker.getTypeAtLocation(node.parent.name);
